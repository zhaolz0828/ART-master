...
M,MV_result,Ground_Truth=dataProcess.Process(n_states,'SP.csv')
AdjaciencyMatrix,LaplacianMatrix=dataProcess.Process_SocialNetwork(n_agents,"SP_Topology.csv",'From_adjaciency_matrix')
...
def train(data, learning_rate=0.001,training_epochs=10, display_step=1):
    ...
    for epoch in range(training_epochs): 
        ...
        if epoch > 330 and cost_val[-1] > np.mean(cost_val[-10:-1]): #330 SP,
